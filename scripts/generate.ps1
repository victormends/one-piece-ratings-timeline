...
<body>
  ...
  <script>
    ...
    function matchesFilters(e) { return typeFilter.has(e.category) && sagaFilter.has(e.saga) && subSagaFilter.has(e.subSaga); }
    function activeEpisodes() { return episodes.filter(matchesFilters).sort((a, b) => a.sortKey - b.sortKey); }
    ...
    function render() {
      ...
      if (!shown.length) {
        ...
        const best = shown.reduce((top, e) => e.rating > top.rating ? e : top, shown[0]);
        document.querySelector("#count").textContent = shown.length; document.querySelector("#average").textContent = avg(shown).toFixed(2); document.querySelector("#best").textContent = `${best.displayCode} (${best.rating.toFixed(1)})`;
        status.textContent = "";
      }
      ...
      for (const saga of sagas) {
        ...
        for (const run of runs) {
          const sub = run.sub;
          const subEpisodes = run.episodes;
          ...
          for (const episode of subEpisodes) {
    ...
    document.querySelector("#reset").addEventListener("click", () => { typeFilter.selectAll(); sagaFilter.selectAll(); subSagaFilter.selectAll(); render(); });
